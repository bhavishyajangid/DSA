...
12; //Given an array arr[] and an integer target, determine if there exists a triplet in the array whose sum equals the given target.
...
function hasTripletSum(arr, target) {
  ...
  for (let j = 0; j < arr.length - 1; j++) {
    for (let k = j + 1; k < arr.length; k++) {
      let diff = target - (arr[j] + arr[k]);
      console.log(arr[j], arr[k], diff);

      if ((obj[diff] && diff !== arr[j] && diff !== arr[k])) {
        console.log(arr[j],arr[k] , diff , 'true');
